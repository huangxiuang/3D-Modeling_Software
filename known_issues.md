...
**问题描述**:
路径生成的位置不是我鼠标点的位置，差的挺多，就连我手动输入坐标后，路径生成的位置也不对。是不是我输入的坐标和使用的坐标是不同的坐标？
...
比如在scene.py里，地形创建用了什么方法？terrian是高斯法？三角函数？还是什么别的方法。aircraft是怎么建模的？
比如多个物体粘合在一起，是怎么实现的？机身？机翼？机头？等等。其他.py也按照这个思路写详细编程实现过程和算法。
...
- **严重程度**: 🟢 轻微     
- **发现日期**: YYYY-MM-DD
**问题描述**:把原来的‘或输入坐标’改成“精准添加路径点”，点击后出来一个弹窗，弹窗里要有一个2D坐标图，用户可以
再图上点击来生成坐标，或者可以直接输入坐标，这一步完成后弹跳到新的窗口选择高度，用户可以选择在1D高度坐标图上点击坐标或者直接输入z坐标高度，然后点击“确定”按钮，这一步完成后，路径点就添加到了路径规划中，并且可以显示在地图上。
...
| ID | 标题 | 模块 | 严重程度 | 状态 |
|----|------|------|----------|------|
id 16 | 添加一种添加路径点的方式 | 路径规划 | 🟢 轻微 | 待修复 |
<!-- 所有已知问题已修复 -->
