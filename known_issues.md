...
| ID-41 | 路径点/飞行/编队/删除Bug修复+流程优化 | 路径规划/飞行动画/编队 | 2026-07-08 | v2.5 |
| ID-42 | 飞行系统多机并发重构+编队完全重建+删除/延迟/对话框修复 | 飞行动画/编队/路径规划 | 2026-07-08 | v2.5 |

---

### [ID-42] 飞行系统多机并发重构 + 编队完全重建 + 多项崩溃修复 ✅
...
| 旧架构 | 新架构 |
|--------|--------|
| 单 `_flight_timer` + 全局状态 | `_flights` dict: 每架飞机独立 QTimer + 独立状态 |
| `_start_flight` 检测 `_flight_active`→`_stop_flight` | 多机同时飞，各进各的 tick |
| `_flight_tick()` 无参 | `_flight_tick(name)` 参数化 |
| `_stop_flight()` 全停 | `_stop_flight(name=None)` 停一架或全停 |

**2. 编队系统完全重建**:
- 编队不再是 toggle 模式，而是创建独立编队对象
- 左侧飞行平台出现"编队1 (aircraft+aircraft2)"树节点
- 子节点: "为编队选择路径点（双击）" + "编队开始飞行（双击）"
- 自定义XY图路径直接分配给编队，不再弹窗问给谁
- "取消编队" → 删除树节点 → 恢复原始路径点

**3. 崩溃修复**:
| # | 崩溃 | 根因 | 修复 |
|---|------|------|------|
| 1 | 延迟飞行选完确认程序崩溃 | `QDoubleSpinBox`→`QTimer`，未调用 `.value()` | `sb.value()` 转 float |
| 2 | 双击编队飞行崩溃 | `_route_slot_action` 引用已删除的 `_on_formation_toggled` | 改为 `_start_formation_dialog()` |
| 3 | 飞行不飞（开始后瞬停） | `_catmull_rom_position` + `_lerp_angle` 被删除 | 重新加入静态方法 |
| 4 | 删除飞机崩溃 | `_formation_aircraft` 已被 `_formations` 替代 | 遍历 `_formations` |
| 5 | 编队选择路径后子节点全消失 | `_refresh_waypoint_tree` 把编队当飞机清了 | 检查 `ds.scene_obj_name` 跳过编队 |
| 6 | 删除飞机后其他飞机树节点消失 | `_populate_aircraft_nodes` 清除所有子节点 + 刷新顺序错误 | 只删飞机节点 + `populate`→`refresh` 顺序修正 |

**4. 对话框修复**:
- `_select_formation_path`: `setattr`→`result_ref[0]` 闭包，OK/确定正常工作
- `_open_flight_aircraft_dialog`: `_flight_active` 检查改为 `_flights`
- 编队树节点 `is_deletable=True`，支持右键删除

**5. _execute_flight 签名修正**:
```python
# 旧: 延迟时 lambda 闭包可能出错
# 新: 直接调用 _start_flight(name)，QDoubleSpinBox.value() → float
```

---

### [ID-41] 路径点/飞行/编队/删除Bug修复 + 流程优化 ✅
...
| # | 问题 | 修复 |
|---|------|------|
| 1 | 精准添加4个路径点 → 弹出4次飞机选择窗口 | `_add_waypoints_batch()`: 批量添加，只弹一次窗口 |
| 2 | 开始飞行弹窗不支持勾选多飞机 | 改为 QCheckBox 多选 + 动态延迟选项（>1架时出现） |
| 3 | 编队飞行无法选择路径点来源 | `_open_formation_flight_dialog()`: 可选某飞机路径点或自定义XY图 |
| 4 | 编队无法取消/恢复 | 新增 `_cancel_formation()`: 恢复原始路径点 + 清编队状态 |
| 5 | 编队UI只有toggle按钮 | 新增"开始编队"/"取消编队"两个独立按钮 |
| 6 | 删除飞机2后，飞机1树状图路径点消失 | `_delete_aircraft`→`_refresh_waypoint_tree()`+`_populate_aircraft_nodes()` |
| 7 | 删除飞机2后，飞机2的飞行预览线残留 | `_delete_aircraft`→清理`_path_actor` |
| 8 | 删除飞机2后，编队列表残留 | `_delete_aircraft`→清理`_formation_aircraft`+`_formation_offsets` |

**延迟出发功能**:
- 勾选多架飞机 → 显示"启用延迟出发"选项
- 可设置每架飞机（除第一架外）的延迟秒数（0-300s）
- 使用 QTimer.singleShot 实现非阻塞延迟

**编队系统 v2.5.1（完全重建）**:

1. 创建编队: 属性面板 → "开始编队" → 勾选 aircraft → 树节点 "编队1 (aircraft+aircraft2)"
2. 编队子节点: "为编队选择路径点（双击）" + "编队路径点 1/2/3..." + "编队开始飞行（双击）"
3. 为编队选择路径点: 可选飞机1/2的路径点 或 自定义XY图（直接给编队，不弹窗问给谁）
4. 编队开始飞行: 双击直接飞，不弹窗
5. 编队僚机跟随: 每tick计算 follower_pos = leader_pos - trail_dist * heading_vector
6. 取消编队: 树节点删除 + 路径点恢复 + UI重置
7. 删除编队成员 → 自动取消整个编队

**多机并发飞行 v2.5.1**:
- `_flights` dict: 每架飞机独立 QTimer + 独立状态
- `_flight_tick(name)`: 参数化，每机独立推进
- `_stop_flight(name=None)`: 停一架或全停
- 延迟出发: 多选飞机 → 设延迟秒数 → QTimer.singleShot 非阻塞
- 编队僚机跟随位置: `leader_pos - trail * d * (cos(yaw), sin(yaw), 0)`

**关键崩溃修复**:
- `_tl_slider.setValue(0)` → `_on_tl_seek` 重置首架飞机状态 → `blockSignals`
- `_catmull_rom_position` + `_lerp_angle` 删除导致飞行不飞 → 恢复
- `_refresh_waypoint_tree` 清编队子节点 → 加 `ds.scene_obj_name` 守卫
- `_populate_aircraft_nodes` 清所有子节点 → 只删飞机节点
